...
public class GenerateReplayScriptForCapacity {  
	...
	if (workloadData.size() > 0) {
		...
		String queue;
		if(inputPartitionSamples.size() <= 8){
		...
		toWrite = "" +  "hdfs dfs -rm -r -skipTrash " + outputPath + "\n";
		...
		if (totalDataPerReduce > 0) {
		    ...
		    toWrite =
                        "" + hadoopCommand + " jar " + pathToWorkGenJar + " org.apache.hadoop.examples.WorkGen" + 
						" " + "-D mapreduce.job.queuename=" + queue + " " + "-conf " + pathToWorkGenConf + " " +
	...
	if (args.length < 10) {
	    ...
	    System.out.println();
	    System.out.println("java GenerateReplayScriptForCapacity");
